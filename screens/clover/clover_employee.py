...
import pandas as pd
from screens.clover.clover_utils.employees import (
    fill_employee,
    fill_user_roles,
)
...
SAVE_FILE_PATH = "clover Employee.xlsx"
...
def map_roles(employee: pd.DataFrame, roles_available: list) -> pd.DataFrame:
    ...
    for job_desc_list in employee['jobDescription']:
        if isinstance(job_desc_list, list):
            ...
            unique_job_descriptions.add(job_desc_list)
...
def process_data(employee, user_roles, role):
    ...
    uploaded_employee_file = st.file_uploader(
        "Upload Employee File (CSV or XLSX)", type=["csv", "xlsx"])
    ...
    if st.session_state['uploaded_employee_file'] is not None:
        ...
        if uploaded_employee_file.name.endswith('.csv'):
            uploaded_employee = pd.read_csv(uploaded_employee_file)
        elif uploaded_employee_file.name.endswith('.xlsx'):
            uploaded_employee = pd.read_excel(
                uploaded_employee_file, sheet_name=0)
...
def clover_emp():
    ...
    st.title("Clover Employee Transformation")
